...
public class SysMenuController extends AbstractController {
    ...
    private void validateFrom(SysMenuDO menu) {
        ...
        if (ObjectUtil.equal(type, MenuTypeEnum.MENU.getValue())) {

            if (StrUtil.isBlank(menu.getUrl())) {
                throw new FastRenException(Status.OPERATE_MENU_ERROR);
            }

            if (!ObjectUtil.equal(parentType, MenuTypeEnum.CATALOG.getValue())) {
                throw new FastRenException(Status.OPERATE_MENU_ERROR);
            }
        }

        // 按钮
        if (ObjectUtil.equal(type, MenuTypeEnum.BUTTON.getValue()) && !ObjectUtil.equal(parentType, 1)) {
            throw new FastRenException(Status.OPERATE_MENU_ERROR);
        }
    }
