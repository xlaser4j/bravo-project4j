...
import com.xlaser4j.opening.modules.sys.form.LoginForm;
...
public class SysLoginController extends AbstractController {
    ...
    public ApiResponse login(@RequestBody LoginForm form) {
        ...
        if (flag) {
            throw new FastRenException(Status.CAPTCHA_NOT_MATCH);
        }
        ...
        if (user == null || StrUtil.equals(user.getPassword(), new Sha256Hash(form.getPassword(), user.getSalt()).toHex())) {
            throw new FastRenException(Status.ACCOUNT_NOT_MATCH);
        }

        // 状态
        if (user.getStatus() == 0) {
            throw new FastRenException(Status.STATUS_LOCKED);
        }
